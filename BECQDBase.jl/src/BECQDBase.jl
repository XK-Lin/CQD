...
This module defines important structures and functions for CQD simulations with Bloch equations.
Author: Xukun Lin
Update: 09/16/2024

Required packages: "Pkg", "Dates", "LinearAlgebra", "Statistics", "Logging", "StatsBase", "DifferentialEquations", "Plots", "DataStructures", "DataFrames", "CSV", "LaTeXStrings", "JSON3".
Required constants: μ₀, γₑ, γₙ.
=#

module BECQDBase

using Pkg, Dates, LinearAlgebra, Statistics, Logging, StatsBase, DifferentialEquations, Plots, DataStructures, DataFrames, CSV, LaTeXStrings, JSON3
...
- `initial_μₑ_direction::String`: The initial condition for the electron magnetic moments. Choose from `"up"` and `"down"`.
- `initial_μₙ_sampling::String`: The sampling method for nuleus magnetic moments. Choose from `"heart shape"`, `"inverse heart shape"`, `"isotropic"`, and `"collapsed"`.
- `solver`: The differential equation solver. Several good ones are `radau()`, `radau5()`, `RadauIIA5()`, and `TRBDF2()`.
- `θₙ_is_fixed::Bool`: (Optional) Whether `θₙ` is fixed or not. Defaults to `true`.
...
- `probability_weight::Vector{Float64}`: (Optional) The weight for collapsed nucleus sampling. Defaults to `[1/4, 1/4, 1/4, 1/4]` (corresponding to `mᵢ = [-3/2, -1/2, 1/2, 3/2]`).
...
struct Simulation
    ...
    initial_μₑ_direction::String
    initial_μₙ_sampling::String
    solver
    θₙ_is_fixed::Bool
    ...
    probability_weight::Vector{Float64}
    ...
    function Simulation(
        ...
        initial_μₑ_direction ∈ ["up", "down"] || throw(ArgumentError("The initial μₑ direction must be either up or down."))
        initial_μₙ_sampling ∈ ["heart shape", "inverse heart shape", "isotropic", "collapsed"] || throw(ArgumentError("The initial μₙ sampling method is invalid. See help of `Simulation`."))
        ...
        (sum(probability_weight) ≈ 1 && all(x -> 0 <= x <= 1, probability_weight)) || throw(ArgumentError("The probability weight is invalid."))
        ...
        inconsistent_combinations = Set([
            ("quadrupole", "down", "inverse heart shape"),
            ("quadrupole", "up", "heart shape"),
            ("exact", "down", "heart shape"),
            ("exact", "up", "inverse heart shape")
        ])
        if (magnetic_field_computation_method, initial_μₑ_direction, initial_μₙ_sampling) ∈ inconsistent_combinations
            @warn "The initial conditions are inconsistent with CQD postulates."
        end
        new(atom_number, magnetic_field_computation_method, initial_μₑ_direction, initial_μₙ_sampling, solver, θₙ_is_fixed, branching_condition, BₙBₑ_strength, δθ, kᵢ, average_method, probability_weight, θ_cross_is_detected, θ_cross_detection_method, detection_start_time, detection_period, B_SG, y_SG)
    end
end
...
function sample_atom_once(simulation::Simulation)
    ...
    θₑ₀ = (simulation.initial_μₑ_direction == "up") ? 0.0 : float(π)
    ϕₑ₀ = 0.0
    if simulation.initial_μₙ_sampling == "heart shape"
        θₙ₀, ϕₙ₀ = 2asin(rand()^(1/4)), 2π * rand()
    elseif simulation.initial_μₙ_sampling == "inverse heart shape"
        θₙ₀, ϕₙ₀ = π - 2asin(rand()^(1/4)), 2π * rand()
    elseif simulation.initial_μₙ_sampling == "isotropic"
        θₙ₀, ϕₙ₀ = 2asin(sqrt(rand())), 2π * rand()
    else
        mᵢ = StatsBase.sample([-3/2, -1/2, 1/2, 3/2], StatsBase.ProbabilityWeights(simulation.probability_weight))
        Bₙ /= (abs(mᵢ) == 1/2) ? 3 : 1
        θₙ₀, ϕₙ₀ = (mᵢ > 0 ? 0.0 : float(π), 0.0)
    end
    return [θₑ₀, θₙ₀, ϕₑ₀, ϕₙ₀, Bₙ, Bₑ]
end
...
"""
function is_flipped(angles::Union{Vector{Float64}, Vector{Vector{Float64}}}, simulation::Simulation)
    angles = angles isa Vector{Float64} ? [angles] : angles
    flips = falses(length(angles))
    for i in eachindex(angles)
        θₑf, θₙf, ϕₑf, ϕₙf = angles[i]
        if simulation.branching_condition == "original"
            θ_comparison = (simulation.initial_μₑ_direction == "up") ? (θₑf > θₙf) : (θₑf < θₙf)
        else
            μₑ_unit_vector = [sin(θₑf) * cos(ϕₑf), sin(θₑf) * sin(ϕₑf), cos(θₑf)]
            μₙ_unit_vector = [sin(θₙf) * cos(ϕₙf), sin(θₙf) * sin(ϕₙf), cos(θₙf)]
            θₑB = θₑf
            θₑₙ = acos(μₑ_unit_vector ⋅ μₙ_unit_vector)
            θ_comparison = (simulation.initial_μₑ_direction == "up") ? (θₑB > θₑₙ) : (θₑB < θₑₙ)
        end
...
"""
    save_results(experiment::Experiment, simulation::Simulation, result::Result, start_time, file_dir)

Save the results of the whole simulation.
"""
function save_results(experiment::Experiment, simulation::Simulation, result::Result, start_time, file_dir)
    ...
    metadata = DataStructures.OrderedDict(
        ...
        "μₑ Sample" => simulation.initial_μₑ_direction,
        "μₙ Sample" => simulation.initial_μₙ_sampling,
    )
    if simulation.initial_μₙ_sampling == "collapsed"
        metadata["μₙ Sample Probability Weight"] = simulation.probability_weight
    end
